...
namespace phy {
  ...
  struct PhyloTree {
    /* Constructor **/
    PhyloTree(StateMap const & alphabet, shared_ptr<BaseTRRateMatrix> const & rmPtr, TreeInfo const & treeInfo, StateMaskMapSet const & stateMaskMapSet, TrctmcFactorSet const & factorSet, DFG const & dfg)
    ...
    shared_ptr<BaseTRRateMatrix> rmPtr;  // rate matrix pointer
  ...
  TrctmcFactorSet mkTrctmcFactorSet(TreeInfo const & treeInfo, shared_ptr<BaseTRRateMatrix> rateMatrixPtr, number_t rateScale = 1);
