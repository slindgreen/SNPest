...
namespace phy {
  ...
  struct SeqToVarSymbol
  {
    /**SeqToVarSymbol constructor. The negative default values will be
       ...
       numeric_limits min values are used as default values in some
       cases to be able to determine when these were not set by the user.*/
    SeqToVarSymbol(string varName, 
		   string seqName, 
		   unsigned size = 1, 
		   int symbolOffset = numeric_limits<int>::min(),
		   int indexOffset = 0, 
		   bool optional = false, 
		   int rSize = numeric_limits<int>::min(), 
		   int rIndexOffset = numeric_limits<int>::min() );
    ...
    int rSize;        // length of right part of di-symbol (in which case "size" then defines left part)
