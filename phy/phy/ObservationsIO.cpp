...
//  }
  ...
  {
    // the int min value is used as the non-allowed default in the SeqToVarSymbol constructor - hence we used it here also 
    int DEFAULT_VALUE = numeric_limits<int>::min();

    string varName;
    string seqName;
    unsigned size = 1;
    int symbolOffset = DEFAULT_VALUE;
    int indexOffset = 0;
    bool optional = false;
    int rSize = DEFAULT_VALUE;
    int rIndexOffset = DEFAULT_VALUE;
